...
def riz_generator(about_text):
    ...
    prompt = "I will provide you a linked in profile that contains the relevant career history and occasionally a short bio that the user writes about themselves. Using a single aspect of the information make a short (no more than a sentance), tailored greeting to use when meeting the person for the first time. This greeting would be used in a business setting and should be mildly flattering. Return only a single greeting in quotation marks. \n Profile Description: " + str(about_text)
...
def text2riz(input):
    text2speach("Looking for a way to greet them now")
...
while True: 
    riz_line = text2riz(input("Who should I riz:  "))
